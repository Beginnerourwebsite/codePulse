  ...
  return new Promise(function (resolve, reject) {
    fetch(apiUrl)
      ...
      .then(function (data) {
        ...
        if (match != "") {
          resolve(
            data.findIndex((x) => x.url.split("./")[1] == match.split("/")[1])
          );
        }
      })
...
window.addEventListener("load", async function () {
  ...
  setTimeout(async () => {
    ...
    if (sessionStoragedata == null) {
      document
        .getElementById(sessionStorage.getItem("active"))
        .setAttribute("class", "active");
    } else {
      if (document.getElementById(sessionStorage.getItem("active")) != null)
        document
          .getElementById(sessionStorage.getItem("active"))
          .setAttribute("class", "active");
    }
